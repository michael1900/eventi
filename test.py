...
def fetch_with_debug(filename, url):
    try:
        ...
        # Se il file è compresso (.gz), decomprimilo
        if filename.endswith('.gz'):
            decompressed_file = filename.replace('.gz', '')
            with gzip.open(filename, 'rb') as f_in, open(decompressed_file, 'wb') as f_out:
                f_out.write(f_in.read())
            print(f'File {filename} decompressed to {decompressed_file}')
            return decompressed_file  # Ritorna il file XML estratto
        else:
            return filename  # Ritorna il file se non è compresso
